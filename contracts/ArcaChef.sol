...
// Arca Token with Governance.
contract ArcaToken is ERC20("Arca Token", "ARCA"), Ownable {
  uint256 private _cap = 100000000e18; //100M
  uint256 private _totalLock;

  uint256 public startReleaseBlock;
  uint256 public endReleaseBlock;
  uint256 public manualMintLimit = 2000000e18; //2M
  ...
  function canUnlockAmount(address _account) public view returns (uint256) {
    // When block number less than startReleaseBlock, no ARCAs can be unlocked
    if (block.number < startReleaseBlock) {
      return 0;
    }
    // When block number more than endReleaseBlock, all locked ARCAs can be unlocked
    else if (block.number >= endReleaseBlock) {
      return _locks[_account];
    }
    // When block number is more than startReleaseBlock but less than endReleaseBlock,
    // some ARCAs can be released
    else
    {
  ...
  function unlock() public {
    require(_locks[msg.sender] > 0, "no locked ARCAs");
  ...
  // @dev move ARCAs with its locked funds to another account
  ...
  function delegateBySig(
    ...
    require(signatory != address(0), "ARCA::delegateBySig: invalid signature");
    require(nonce == nonces[signatory]++, "ARCA::delegateBySig: invalid nonce");
    require(now <= expiry, "ARCA::delegateBySig: signature expired");
  ...
  function getPriorVotes(address account, uint256 blockNumber) external view returns (uint256) {
    require(blockNumber < block.number, "ARCA::getPriorVotes: not yet determined");
  ...
  function _delegate(address delegator, address delegatee) internal {
    ...
    uint256 delegatorBalance = balanceOf(delegator); // balance of underlying ARCAs (not scaled);
  ...
  function _writeCheckpoint(
    ...
    uint32 blockNumber = safe32(block.number, "ARCA::_writeCheckpoint: block number exceeds 32 bits");
...
interface IFairLaunch {
  ...
  function pendingArca(uint256 _pid, address _user) external view returns (uint256);
...
// ArcaChef is a smart contract for distributing ARCA by asking user to stake the ERC20-based token.
contract ArcaChef is IFairLaunch, Ownable {
  using SafeMath for uint256;
  ...
  struct UserInfo {
    ...
    //
    // We do some fancy math here. Basically, any point in time, the amount of ARCAs
    // entitled to a user but is pending to be distributed is:
    //
    //   pending reward = (user.amount * pool.accArcaPerShare) - user.rewardDebt
    //
    // Whenever a user deposits or withdraws Staking tokens to a pool. Here's what happens:
    //   1. The pool's `accArcaPerShare` (and `lastRewardBlock`) gets updated.
  ...
  struct PoolInfo {
    ...
    uint256 allocPoint; // How many allocation points assigned to this pool. ARCAs to distribute per block.
    uint256 lastRewardBlock; // Last block number that ARCAs distribution occurs.
    uint256 accArcaPerShare; // Accumulated ARCAs per share, times 1e12. See below.
    uint256 accArcaPerShareTilBonusEnd; // Accumated ARCAs per share until Bonus End.
  }

  // The Arca TOKEN!
  ArcaToken public arca;
  // Dev address.
  address public devaddr;
  // ARCA tokens created per block.
  uint256 public arcaPerBlock;
  // Bonus muliplier for early arca makers.
  uint256 public bonusMultiplier;
  // Block number when bonus ARCA period ends.
  ...
  // The block number when ARCA mining starts.
  ...
  */
  ...
  function setArcaPerBlock(uint256 _arcaPerBlock) public onlyOwner {
    arcaPerBlock = _arcaPerBlock;
  }
  ...
  function addPool(
    ...
    poolInfo.push(
      PoolInfo({
        ...
        accArcaPerShare: 0,
        accArcaPerShareTilBonusEnd: 0
      })
    );
  }

  // Update the given pool's ARCA allocation point. Can only be called by the owner.
  function setPool(
    uint256 _pid,
    uint256 _allocPoint,
    bool _withUpdate
  ...
  */
  ...
  // View function to see pending ARCAs on frontend.
  function pendingArca(uint256 _pid, address _user) external override view returns (uint256) {
    PoolInfo storage pool = poolInfo[_pid];
    UserInfo storage user = userInfo[_pid][_user];
    uint256 accArcaPerShare = pool.accArcaPerShare;
    uint256 lpSupply = IERC20(pool.stakeToken).balanceOf(address(this));
    if (block.number > pool.lastRewardBlock && lpSupply != 0) {
      uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
      uint256 arcaReward = multiplier.mul(arcaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
      accArcaPerShare = accArcaPerShare.add(arcaReward.mul(1e12).div(lpSupply));
    }
    return user.amount.mul(accArcaPerShare).div(1e12).sub(user.rewardDebt);
  }
  ...
  function updatePool(uint256 _pid) public override {
    ...
    uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
    uint256 arcaReward = multiplier.mul(arcaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    arca.mint(devaddr, arcaReward.div(10));
    arca.mint(address(this), arcaReward);
    pool.accArcaPerShare = pool.accArcaPerShare.add(arcaReward.mul(1e12).div(lpSupply));
    // update accArcaPerShareTilBonusEnd
    if (block.number <= bonusEndBlock) {
      arca.lock(devaddr, arcaReward.div(10).mul(bonusLockUpBps).div(10000));
      pool.accArcaPerShareTilBonusEnd = pool.accArcaPerShare;
    }
    if(block.number > bonusEndBlock && pool.lastRewardBlock < bonusEndBlock) {
      uint256 arcaBonusPortion = bonusEndBlock.sub(pool.lastRewardBlock).mul(bonusMultiplier).mul(arcaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
      arca.lock(devaddr, arcaBonusPortion.div(10).mul(bonusLockUpBps).div(10000));
      pool.accArcaPerShareTilBonusEnd = pool.accArcaPerShareTilBonusEnd.add(arcaBonusPortion.mul(1e12).div(lpSupply));
    }
    pool.lastRewardBlock = block.number;
  }

  // Deposit Staking tokens to ArcaChef for ARCA allocation.
  function deposit(address _for, uint256 _pid, uint256 _amount) public override {
    ...
    user.rewardDebt = user.amount.mul(pool.accArcaPerShare).div(1e12);
    user.bonusDebt = user.amount.mul(pool.accArcaPerShareTilBonusEnd).div(1e12);
    emit Deposit(msg.sender, _pid, _amount);
  }

  // Withdraw Staking tokens from ArcaChef.
  ...
  // Harvest ARCAs earn from the pool.
  ...
  function _harvest(address _to, uint256 _pid) internal {
    ...
    uint256 pending = user.amount.mul(pool.accArcaPerShare).div(1e12).sub(user.rewardDebt);
    require(pending <= arca.balanceOf(address(this)), "wtf not enough arca");
    uint256 bonus = user.amount.mul(pool.accArcaPerShareTilBonusEnd).div(1e12).sub(user.bonusDebt);
    safeArcaTransfer(_to, pending);
    arca.lock(_to, bonus.mul(bonusLockUpBps).div(10000));
  }
  ...
  function emergencyWithdraw(uint256 _pid) public {
    ...
    // Safe arca transfer function, just in case if rounding error causes pool to not have enough ARCAs.
  function safeArcaTransfer(address _to, uint256 _amount) internal {
    uint256 arcaBal = arca.balanceOf(address(this));
    if (_amount > arcaBal) {
      arca.transfer(_to, arcaBal);
    } else {
      arca.transfer(_to, _amount);
    }
  }
